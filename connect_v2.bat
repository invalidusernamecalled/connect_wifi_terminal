...
set band=
set interface_state=
for /f "tokens=1,* delims=:" %%i in ('netsh wlan show interfaces ^| findstr /ir "Name.*[:] State.*[:] ssid.*[:] band.*[:] bssid.*[:]"') do (
...
if !all_ears!==1 for /f "tokens=1,2 delims=: " %%b in ("%%i %%j") do if /i "%%b"=="band" (if /i "%%c"=="2.4" set band=2.4)&(if /i "%%c"=="5" set band=5)
if !all_ears!==1 for /f "tokens=1,2 delims= " %%b in ("%%i") do if /i "%%b %%c"=="ap bssid" for /f "tokens=* delims= " %%a in ("%%j") do set "bssid_connected=%%a"
if !all_ears!==1 for /f "tokens=1 delims= " %%b in ("%%i") do if /i "%%b"=="state" for /f "tokens=* delims= " %%a in ("%%j") do echo interface state is %%a*...&set interface_state=%%a
)
call :picknext
...
if !disconnect_times! GTR 15 choice /c yncdr /d c /t 1 /m "(C)ontinue (D)elet Profile? (R)etry"
if !disconnect_times! GTR 15 (set disconnect_times=0&(if !errorlevel!==1 goto :start)&(if !errorlevel!==2 call :disconnect & goto :nekst)&(if !errorlevel!==4 goto :delete_profile)&(if !errorlevel!==5 call :callrepeat))
)
REM goto repeat_all_ears near the :END
goto :END
:callrepeat
netsh wlan connect name="%%i" ssid=!ssid_[%choice%]! interface="!interfacename!" >NUL & call :checkerrorlevelwlanprofile
exit /b
:nekst
echo Waiting [3 sec] to detect ip address. press key to skip & timeout 3 >NUL
...
exit /b
    :picknext
cls 
echo interface state is %interface_state%...
if "%ssid_connected%"=="" set found_ip_address=
if /i "%interface_state%" NEQ "connected" set found_ip_address=
if /i "%interface_state%" == "connected" if "%ssid_connected%" NEQ "" (if /i "%band%"=="2.4" echo:Connected band  is 2.4 ghz)
if /i "%interface_state%" == "connected" if "%ssid_connected%" NEQ "" (if /i "%band%"=="5" echo:Connected band  is 5 ghz)
if /i "%interface_state%" == "connected" if "%bssid_connected%" NEQ "" echo:Connected bssid is %bssid_connected%
echo interface:^<!interfacename!^>%tab%%tab%%tab%%tab%^( Scanning is%tab%%tab%%title_append%& echo:%tab%%tab%%tab%%tab%%tab%throttled by Windows API^)&(echo|set/p=%tab%IP: [%found_ip_address%], & if %pinged_google%==1 echo Able to ping to google)
    ...
    echo:
    echo:
    if "!interfacename!" NEQ "" (for /f "delims=" %%i in ("!interfacename!") do echo:) else (echo: & echo:***No Wireless interface found^!*** & echo: &  PAUSE & GOTO :eof)
